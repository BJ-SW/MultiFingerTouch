...
Version 1.01 2020/10/29 update
...
class GOEControlClass:
    ...
    def __readVer(self, command):
        try:
            ...
            if Interpreter == '3':
                readState1 = readState1.encode('utf-8')
                readState2 = readState2.encode('utf-8')
    ...
    def SetHomeOffset(self,Value):
        try:
            command = '%01#WDD0223202233'# '%01#WDD0023000231'
            finalByte = self.__flipByte(Value)
            command = command + finalByte
            ret = self.__writeRead(command)
            if (ret == -1):
            ...
            self.strErrorMessage = "SetHomeOffset error"
            return -1
